...
async def test_get_resume_by_user_id(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.matching_service.collection_name", fake_collection
    )
    ...
    assert (
        result["fake_field"] == "fake_value"
    ), f"expected 'fake_field' to be 'fake_value', got '{result['fake_field']}' instead"
